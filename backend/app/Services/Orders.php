...
class Orders{
     ...
     * @throws \Exception
     */
    private static function _getQueryDate(string $timestamp): string
    {
     ...
     * @return void
     */
    public static function csv_row_to_order($row){
        $columns = explode('","', $row);//文章の中にカンマがあるため
        $order = new \App\Models\Orders;
        if(count($columns) <= 1 ) return;//最後のカンマが空の1行とみなされることがあるため
