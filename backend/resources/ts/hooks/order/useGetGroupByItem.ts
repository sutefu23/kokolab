...
const getGroupByItem = async ({
    fromDate,
    toDate,
}: QueryParam): Promise<OrderGroupByItem[]> => {
    const { data } = await axios.get<OrderGroupByItem[]>(
        "/api/orders/group_by_item",
...
const useGetGroupByItemQuery = <TData = OrderGroupByItem[]>(
    ...
    useQuery(
        ["groupByItem", fromDate, toDate],
        () => getGroupByItem({ fromDate, toDate }),
        options
    );
